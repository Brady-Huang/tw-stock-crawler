...
測試股票代號List是否為空，測試股票代號第一個是否為1101以及最後一個為9958，確認有正確抓到股票代碼資訊，
股票代號抓取的第一個個股為台泥(1101)，機率極低會有比這檔股票更前面得上市股票，因此若第一個非1101，則網頁結構可能有變
股票代號最後一個為9958，此代號是有可能變動，但很少發生，主要想測試是否股票代號最後一個抓取到非上市股票的資訊
產業別對應的股票代號是一個dictionary，會有28個分別的產業對應各自的個股，因此只需測試此dictionary是否有28個產業
```

2. 測試存取的listed.json檔是否正確

```
listed.json存取方式為key-value pair，key為個股代號以及column，為了確認有抓到個股收盤資訊，
```
